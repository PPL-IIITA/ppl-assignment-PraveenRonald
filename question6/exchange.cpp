...
void make_couple(boy b[],girl g[],couple z[])
{
    ...
    int x,y;
	for(x=0;x<j;x++)//algo for making couple
	{
...
void gift_exchange(gift a[],couple z[])
{
	...
	string name,type;
	while(!(fp.eof()))//taking input from file
	{
...
void breakup(couple z[],boy b[],girl g[],int t)
{
	int i;
	fstream file;
	file.open("break_up.txt");
	cout <<"couples with happiness less than "<<t<<endl;
	for(i=0;i<60;i++)
	{
		if(z[i].get_happiness_of_couple()<t)
		{
			if(z[i].get_break_up()!=true)
			{
	...
	for(i=0;i<60;i++)//performs breakup
	{
		if(z[i].get_break_up()!=true)
		if(z[i].get_happiness_of_couple()<t)
		{
