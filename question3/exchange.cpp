...
void input()
{
	...
	string types[]={"Essential_Gift","Luxury_Gift","Utility_Gift"};
	for(i=0;i<n;i++)
	{
		type=rand()%3;
		price=rand()%10+15;
	...
	string commited_type[]={"The_Miser","The_Generous","The_Geek"};
	for(i=0;i<n;i++)
	{
		attrac=rand()%400+200;
		budget=rand()%40+20;
	...
	string commited_type_girl[]={"The_Choosy","The_Normal","The_Desperate"};
	string type_of_girl;
...
void gift_exchange(gift a[],boy b[],girl g[],couple z[])
{
	fstream f;
	f.open("boy_data.txt");
	...
	fstream fp;
	fp.open("gift_data.txt");
	int price,value,lux_rating,difficulty,util_val,util_class,m=0;
	while(!(fp.eof()))
	{
	...
	for(i=0;i<60;i++)
	{
		int temp=abs(z[i].get_maintenance_cost()-z[i].get_budget())+abs(z[i].get_boyintel()-z[i].get_girlintel())+abs(z[i].get_boyattractiveness()-z[i].get_girlattractiveness());
		z[i].set_compatibility_of_couple(temp);
	}
	...
	for(i=0;i<k;i++)
	cout<<z[i].get_boyname()<<" "<<z[i].get_girlname()<<" "<<z[i].get_happiness_of_couple()<<endl;
	//sort according to compatibility of couple
	for(i=0;i<60;i++)
	{
		for(j=0;j<60;j++)
		{
			if(z[i].get_compatibility_of_couple()>z[j].get_compatibility_of_couple())
			{
				temp1=z[i];
				z[i]=z[j];
				z[j]=temp1;
	...
	for(i=0;i<k;i++)
	cout <<z[i].get_boyname()<<" "<<z[i].get_girlname()<<" "<<z[i].get_compatibility_of_couple()<<endl;
}
